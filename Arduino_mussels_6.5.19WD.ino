...
// WD 04jun2019 these come in as digits in inputData, and should be Double to match how desiredM1, etc were declared    
...
//    Serial.println("");
    ...
    dataLength = 0; // reset counter for dataLength  WD NOTE: MAYBE DON'T DO THIS?
...
void heatControl(double desiredTemp, double actualTemp, int heatPin) {
  double targetOut;
  double perDiff = percentDiff(desiredTemp, actualTemp); // WD 05jun2019 had other way around (desiredTemp, actualTemp);
  double slope = 1; //WD 05jun2019 was = 255/100; //changed so yint is 0 - shouldn't they always go through 0??
...
//  }
//  else {
//    targetOut = output;// + 90;  //WD NOTE: Why add 90 here? 
//  }
//  if (targetOut > 255) {
//    targetOut = 255;
//  }
  timeout = 2000*(perDiff+.03);  // wait 2000 ms * %difference, if far from setpoint, stay on longer
                                // added 0.1 b/c never quite getting there on high temps at short on cycle
...
double percentDiff(double x, double y) {  //x = desired; y = actual;
  if (y <0) { y = 50;} // if no thermistor on channel, trick into thinking channel is hot & set PWM output to zero 
  double x2 = x + 273.15;
  double y2 = y + 273.15; //put on absolute K scale for percentage calculation???
  //double result = (x - y)/(y) * 100;
  double result = ((y2 - x2)/x2)*100;  //WD 05jun2019 (actual-desired)/desired; 
  if (result > 0){   //if positive above setpoint, heating proportion = 0; 
    result = 0;}
  else {result = result*(-1);}  //if negative, flip value
  return result;
}
